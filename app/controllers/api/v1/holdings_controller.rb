...
class Api::V1::HoldingsController < Api::V1::BaseController
  ...
  private
    ...
    def apply_filters(query)
      ...
      query = query.where(account_id: Array(params[:account_ids])) if params[:account_ids].present?
      query = query.where(date: parse_date!(params[:date], "date")) if params[:date].present?
      query = query.where("holdings.date >= ?", parse_date!(params[:start_date], "start_date")) if params[:start_date].present?
      query = query.where("holdings.date <= ?", parse_date!(params[:end_date], "end_date")) if params[:end_date].present?
      query = query.where(security_id: params[:security_id]) if params[:security_id].present?
      query
    end
