...
class Api::V1::TradesController < Api::V1::BaseController
  ...
  def index
    ...
    render_validation_error(error.message, [ error.message ])
  rescue => error
    log_and_render_error("index", error)
  end

  def show
    render :show
  rescue => error
    log_and_render_error("show", error)
  end
  ...
  def create
    ...
    message = (error.model == "Account") ? "Account not found" : "Security not found"
    render json: { error: "not_found", message: message }, status: :not_found
  rescue => error
    log_and_render_error("create", error)
  end
  ...
  def update
    ...
    log_and_render_error("update", error)
  end
  ...
  def destroy
    ...
    log_and_render_error("destroy", error)
  end
  ...
  private
    ...
    def apply_filters(query)
      ...
      query = query.where(entries: { account_id: params[:account_id] }) if params[:account_id].present?
      query = query.where(entries: { account_id: Array(params[:account_ids]) }) if params[:account_ids].present?
      query = query.where("entries.date >= ?", parse_date!(params[:start_date], "start_date")) if params[:start_date].present?
      query = query.where("entries.date <= ?", parse_date!(params[:end_date], "end_date")) if params[:end_date].present?
      query
    end
    ...
    def trade_sell_from_type_or_nature?(value)
      ...
      %w[sell inflow].include?(value.to_s.downcase.strip)
    end
